...
class AlignedDataset(BaseDataset):
    def initialize(self, opt):
        ...
        if opt.use_colorjitter_contrast:
            contrast_param = (0.3, 1)
        else:
            contrast_param = 0
        if opt.use_colorjitter_saturation:
            saturation_param = (0.3, 1)
        else:
            saturation_param = 0
        
        self.colorjitter1 = tf.ColorJitter(contrast=contrast_param)

        self.colorjitter2 = tf.ColorJitter(contrast=contrast_param,
    ...
    def __getitem__(self, index):        
        ...
        B = Image.open(B_path) #load RGB

        transform_B = get_transform(self.opt, params)
        B_tensor = transform_B(B)

        if not self.opt.isTrain:
            #Test할 때 실제 GT 이미지도 같이 저장하기 위한 코드로 train 시에는 영향 x 
            rgb = B_tensor
            B_tensor = 0
